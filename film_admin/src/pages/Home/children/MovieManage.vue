          ...
          async addImg(e){
            ...
            if (json.state==200){
              ...
              if (this.dialogTitle === '编辑电影信息') {
                this.movieInfo.photosList.push("http://localhost:8080/upFile/"+json.data.img);
              } else{
                this.newPhotos.push("http://localhost:8080/upFile/"+json.data.img);
              }
            }
          ...
          async manageMovieInfo(){
            if (this.movieInfo.nm&&this.movieInfo.img&&this.movieInfo.dir&&this.movieInfo.star&&this.movieInfo.dur&&this.movieInfo.cat&&this.movieInfo.language&&this.movieInfo.rt&&this.movieInfo.dra) {
              ...
              if (json.state===200){
                ...
                if (json.data){
                  if(json.data.img)
                    this.movieInfo.img = 'http://localhost:8080/upFile/'+json.data.img;
                  if(json.data.videoImg)
                    this.movieInfo.videoImg = 'http://localhost:8080/upFile/'+json.data.videoImg;
                  if(json.data.video)
                    this.movieInfo.videoUrl = 'http://localhost:8080/upFile/'+json.data.video;
                }
                //判断是编辑还是添加
                if (this.dialogTitle === '编辑电影信息') {
